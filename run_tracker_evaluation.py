...
def evaluate():
    ...
    if evaluation.video == 'All':
        ...
        for i in range(nv):
            print(videos_list[i])
            save_video_folder = os.path.join(cwd, 'tracked data', videos_list[i])
...
#            frame_name_list_ = frame_name_list[0:]
            ...
            idx = i
            bboxes, speed[idx] = tracker(hp, run, design, frame_name_list, save_video_folder, pos_x, pos_y, target_w, target_h, final_score_sz,
                            image, templates_z, scores, path_ckpt = os.path.join(design.saver_folder, design.path_ckpt), siamNet = siamNet)
            bboxes = _extract_bbox(env, evaluation, videos_list[i])
            lengths[idx], precisions[idx], precisions_auc[idx], ious[idx] = _compile_results(gt_, bboxes, evaluation.dist_threshold)

            Round_BBoxes = np.round(bboxes).astype(int)
            video_folder = os.path.join(env.root_dataset, evaluation.dataset, videos_list[i])
            if not os.path.exists(os.path.join(os.getcwd(),"results")):
                os.makedirs(os.path.join(os.getcwd(),"results"))
            with open(os.path.join(os.getcwd(),"results", videos_list[i]) + ".txt","w+") as f:
              for row in Round_BBoxes:
...
#              ' -- Speed: ' + "%.2f" % mean_speed + ' --')
    #evaluate only one vedio
        ...
        save_video_folder = os.path.join(env.root_dataset, evaluation.dataset, evaluation.video)
	
        pos_x, pos_y, target_w, target_h = region_to_bbox(gt[0])
        bboxes, speed = tracker(hp, run, design, frame_name_list, save_video_folder, pos_x, pos_y, target_w, target_h, final_score_sz,
	                            image, templates_z, scores, path_ckpt = os.path.join(design.saver_folder, design.path_ckpt), siamNet = siamNet)
...
def _init_video(env, evaluation, video):
    ...
    line = gt_f.readline()
    deli = detect(line)
    gt = np.genfromtxt(gt_file, delimiter=deli, usecols=np.arange(0,4))
